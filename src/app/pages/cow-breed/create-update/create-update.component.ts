...
export class CreateUpdateComponent implements OnInit, OnDestroy {
  ...
  removePeriod(index: number, period: any, e: any) {
    e.preventDefault()
    if (!!this.isCreate || !this.cowBreed['periods'] || index > this.cowBreed['periods'].length - 1) {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      ...
      if (action === 'delete') {
        this.cowBreedService.deletePeriod(_id).subscribe((res) => {
          const { status } = res
          if (status === true) {
            this.commonService.openAlert('Xoá giai đoạn sinh trưởng thành công', 'success')
  ...
  onSubmit() {
    ...
    if (!!this.isCreate) {
      ...
      this.submitted = true
      this.cowBreedService.updateCowBreed(this.cowBreedId, this.form.value).subscribe((res) => {
        const { status } = res
        if (!status) {
          this.submitted = false
          this.setValueForForm(this.cowBreed)

...
export class CreateMealComponent implements OnInit {
  ...
  form: FormGroup = new FormGroup({
    ...
    idsFood: new FormControl({ value: [], disabled: true }, [Validators.required]),
  })
  period!: any
  areas: any[] = []
  foods: any[any] = []
  ...
  getCowBreeds(): void {
    this.loading = true
    this.cowBreedService.fetchCowBreeds().subscribe((res: any) => {
      const { data } = res
  ...
  getPeriods(): void {
    this.periods = []
    this.cowBreedService.fetchCowBreed(this.selectedCowBreed).subscribe((res: any) => {
      const { data } = res
  ...
  getPeriod(): void {
    this.cowBreedService.fetchPeriod(this.selectedCowBreedPeriod).subscribe((res: any) => {
      const { status, data } = res
      if (!status) {
        return
      }
      ...
      const nutrition = data.nutrition.map((n: any) => {
        const description = INITIAL_PERIOD_NUTRITION.find((field: any) => field.name === n.name)?.description
        return {
          ...n,
          description,
        }
      ...
      this.period = period
      const nutritionValueZero = period.nutrition.find((item: any) => item.amount === 0)
